...
# Function to remove the non-ASCII characters
...
def product_assistant(ascii_transcript):
    ...
    However, be aware that some acronyms can have different meanings based on the context (e.g., 'LTV' can stand for 'Loan to Value' 
    or 'Lifetime Value'). You will need to discern from the context which term is being referred to and apply the appropriate transformation. 
    In cases where numerical figures or metrics are spelled out but do not represent specific financial products 
    (like 'twenty three percent'), these should be left as is. Your role is to analyze and adjust financial product
     terminology in the text. Once you've done that, produce the adjusted transcript and a list of the words you've changed.
    """
    ...
    new_model_id="meta-llama/llama-3-2-11b-vision-instruct"
    params = TextChatParameters(
        temperature=0.2, # 比較不會亂回答
        top_p=0.6 # 控制產出的多樣性
    )
...
def transcript_audio(audio_file):
    ...
    with open(output_file, "w") as file: # 這個"w"是指write模式，就算上面有東西也會複寫過去
        file.write(result)

    # Return the textual result and the file for download
    return reuslts, output_file
